...
describe Day9 do
  ...
  describe Day9::FileSystem do
    let(:expected_output_1){ '0..111....22222' }
    let(:expected_output_2){ '02.111....2222.' }
    let(:expected_output_3){ '022111....222..' }
    let(:expected_output_4){ '0221112...22...' }
    let(:expected_output_5){ '02211122..2....' }
    let(:expected_output_6){ '022111222......' }

    describe ".from_diskmap" do
      it 'parses the diskmap into file block positions' do
        actual = Day9::FileSystem.from_diskmap('2333133121414131402')
    ...
    describe "#all_free_space_contiguous?" do
      it 'returns true when there is no free space' do
        assert Day9::FileSystem.new('001122').all_free_space_contiguous?
      end
      it 'returns true when there is no gap in the free space' do
        assert Day9::FileSystem.new(expected_output_6).all_free_space_contiguous?
      end
      it 'returns false when there are gaps in the free space' do
        refute Day9::FileSystem.new(expected_output_5).all_free_space_contiguous?
      end
      it 'returns false if the freespace is all at the beginning' do
        refute Day9::FileSystem.new(expected_output_6.reverse).all_free_space_contiguous?
      end
    end

    describe "#checksum" do
      it 'sums the values of block position x fileID' do
        filesystem = Day9::FileSystem.new('0099811188827773336446555566..............')
        expected = 1928
    ...
    describe "#compact! moves file blocks one at a time from the end of the disk to the leftmost free space block until all free space is contiguous" do
      it 'compacts the short example' do
        fs = Day9::FileSystem.new(expected_output_1)
        fs.compact!
        _(fs.to_s).must_equal expected_output_6
      end

      it 'compacts the long example' do
        fs = Day9::FileSystem.from_diskmap(overall_scenario_input)
    ...
    describe "#compact_once! moves file blocks one at a time from the end of the disk to the leftmost free space block" do
      ...
      it 'moves from state 2 to state 3' do
        fs = Day9::FileSystem.new(expected_output_2)
        fs.compact_once!
        _(fs.to_s).must_equal expected_output_3
      end

      it 'moves from state 3 to state 4' do
        fs = Day9::FileSystem.new(expected_output_3)
        fs.compact_once!
        _(fs.to_s).must_equal expected_output_4
      end

      it 'moves from state 4 to state 5' do
        fs = Day9::FileSystem.new(expected_output_4)
        fs.compact_once!
        _(fs.to_s).must_equal expected_output_5
      end

      it 'moves from state 5 to state 6' do
        fs = Day9::FileSystem.new(expected_output_5)
        fs.compact_once!
        _(fs.to_s).must_equal expected_output_6
      end
    end
    ...
    describe "#to_s" do
      it 'outputs a string representation of the file block positions' do
        actual = Day9::FileSystem.new('00...111...2...333.44.5555.6666.777.888899')
        expected = '00...111...2...333.44.5555.6666.777.888899'
        _(actual.to_s).must_equal(expected)
      end
    end
